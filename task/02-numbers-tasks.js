...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let scalarProductVectors = (x1 * x2) + (y1 * y2);
    let modulVectorsFirst = Math.hypot(x1, y1);
    let modulVectorsSecond = Math.hypot(x2, y2);
    let cosAngleVectors =  scalarProductVectors / (modulVectorsFirst * modulVectorsSecond);
    return Math.acos (cosAngleVectors);
}
...
function roundToPowerOfTen(num, pow) {
    let tenPower = Math.pow(10, pow);
    let result = num / tenPower;
    return Math.round(result) * tenPower;
}
